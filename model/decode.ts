...
export function decode(passcode: string): IDecodeResult {
    ...
    const checksumActual = (
        ...
        decodedValues[3] +
        decodedValues[4] +
        decodedValues[5]) ^ decodedValues[6];
    ...
    const cash = (
        ((decodedValues[0] << 11) & 0xF000) | /* lower 3 bits, Missing MSB each time? */
    ...
    return {
        ...
        error: isValid ? '' : `Invalid passcode. Checksum mismatch: expected,actual ${checksumExpected},${checksumActual}`,
        isValid,
    }
}